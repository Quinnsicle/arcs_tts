...
local ambitions = {{
    name = "Undeclared",
    row_pos = Vector({0, 0, -1.08})
}, {
    name = "Tycoon",
    row_pos = Vector({0, 0, -0.72})
}, {
    name = "Tyrant",
    row_pos = Vector({0, 0, -0.34})
}, {
    name = "Warlord",
    row_pos = Vector({0, 0, 0.05})
}, {
    name = "Keeper",
    row_pos = Vector({0, 0, 0.43})
}, {
    name = "Empath",
    row_pos = Vector({0, 0, 0.84})
}}
...
function AmbitionMarkers.declare(player_color)
    ...
    local is_faithful = (lead_info.type == "Faithful Zeal" or
                            lead_info.type == "Faithful Wisdom")

    -- Is the lead card a 1?
    if (lead_info.number == 1 and not is_faithful) then
        broadcastToColor("Actions numbered 1 cannot be declared",
            player_color)
        return
    end

    local power = high_marker[high_marker.object.is_face_down]
                      .power_desc

    if (lead_info.number == 7 or is_faithful) then
        broadcastToAll(player_color ..
                           " is declaring ambition of choice for " ..
                           power, player_color)
        ...
        broadcastToAll(player_color .. " has declared " ..
                           this_ambition.name .. " ambition for " ..
                           power, player_color)
    end

    zero_marker.setPositionSmooth(reach_board.positionToWorld({1.02,
                                                               0.2,
                                                               0.67}))
...
function AmbitionMarkers.highestUndeclared()
    ...
    for _, marker in pairs(available_markers) do
        ...
        local this_points = this_marker[this_marker.object
                                .is_face_down].first_power
...
function AmbitionMarkers.resetZeroMarker()
    zero_marker.setPositionSmooth(reach_board.positionToWorld({0.94,
                                                               0.2,
                                                               1.09}))
